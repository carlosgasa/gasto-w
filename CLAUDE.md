...
- **Producción**: https://gasto-w.web.app (Firebase Hosting). Para redesplegar tras un cambio: `npm run build && npx firebase deploy --only hosting`. Para redesplegar reglas de Firestore tras editar `firebase/firestore.rules`: `npx firebase deploy --only firestore:rules`. El CLI de Firebase en esta máquina ya está autenticado como el dueño del proyecto.
...
| 4 | Comparativas e histórico (`monthlySummaries`) | 100% | Comparativa mes vs. anterior, tendencia 12 meses, ranking, barras apiladas por categoría (6 meses) y heatmap de calendario de gasto diario |
| 5 | Insights/predicción | 100% | Rachas de 3+ meses por categoría, comparación % vs. mes anterior, proyección por ritmo de gasto y categoría que más creció, como feed de avisos |
...
| 7 | Respaldo y PDF (export/import JSON, estado de cuenta en PDF) | 100% | Export/import JSON preservando IDs; PDF por mes (histórico o actual "al día de hoy") desde Reportes |
| 8 | Configuración avanzada y PWA polish (temas, zoom, manifest, service worker) | 95% | 3 temas, zoom, manifest válido (incluido `lang: es`) + service worker con íconos del logo, todo verificado en el build de producción. Solo falta que el usuario confirme la instalación en su propio celular/escritorio — eso no se puede probar desde aquí |
...
**Avance global estimado: ~99%** — todo lo que se podía construir y verificar desde aquí está hecho. Lo único pendiente es que el usuario instale la PWA en su propio dispositivo y confirme que se ve/comporta bien (favicon, ícono de app, modo standalone).

## Estado actual

App de React + Vite + TS corriendo en `localhost:5173`, conectada al proyecto Firebase `gasto-w` (Auth + Firestore con persistencia offline, verificada explícitamente: `monthlySummaries` usa incrementos encolables en vez de una transacción para que funcione sin internet). Flujo completo funcionando: login, alta y edición de cuentas/categorías (con selector visual de ícono/color propio, más de 15 colores y ~19 íconos, sin duplicados), captura de gastos puntuales como pantalla de inicio (título = descripción del gasto, con campos extra de combustible), gastos recurrentes que se autogeneran cada mes, dashboard de resumen con totales y gráfica, Reportes con comparativas/tendencia/barras apiladas por categoría/heatmap de calendario/ranking/avisos/combustible y exportación a PDF por mes, y respaldo/restauración en JSON desde Configuración. Selectores de cuenta/categoría en los formularios muestran ícono y color (no `<select>` planos). Navegación con menú hamburguesa en móvil. PWA con manifest, service worker e íconos generados del logo. Reglas de Firestore (`firebase/firestore.rules`) restringidas al UID del dueño, ya desplegadas. **App en producción: https://gasto-w.web.app** (además de `localhost:5173` para desarrollo).

**Bug corregido (2026-09-03)**: `monthlySummaries.byCategory`/`byAccount` no se actualizaban (aunque `totalAmount` sí) porque `setDoc(..., {merge:true})` con una llave de texto tipo `"byCategory.xyz"` la guarda como nombre de campo literal en vez de anidarla — el fix usa un objeto anidado (`byCategory: {xyz: increment(n)}}`), que sí hace merge profundo real. Además se agregó `recomputeAllMonthlySummaries` (en `application/use-cases/recomputeSummaries.ts`), que reconstruye todos los meses desde los gastos reales y corre automáticamente al abrir la app (en `AppLayout`), reparando los datos que ya se habían guardado mal.
